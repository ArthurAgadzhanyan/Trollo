...
/*    <div class="blocks-folder__block blocks-folder__block_droppable">
        <textarea class="block__title" ></textarea>
        <ul class="blocks-folder__block__list">
        </ul>
        <div class="blocks-folder__block__button" id="0">Добавить карточку</div>
    </div>*/
});
...
document.addEventListener("DOMContentLoaded", function () {
    var indexOfButton = '0';
    var BlockItem;
    var ItemPlace = document.getElementsByClassName("blocks-folder__block__list");
    var ButtonCreateBlockItems =  document.getElementsByClassName("blocks-folder__block__button");



   var button1 = new ButtonCreateBlockItem();
...
*/
    ...
    /*    var BlockItem;
        var ItemPlace = document.getElementsByClassName("blocks-folder__block__list");
        var ButtonCreateBlockItem =  document.getElementsByClassName("blocks-folder__block__button");
        var IndexOfBlock = 2;


        for (var i=0;i<ButtonCreateBlockItem.length; i++) {
            ButtonCreateBlockItem[i].onclick = createBlockItem
        }

        function createBlockItem() {
            BlockItem = document.createElement("li");
            BlockItem.className = "list__item";
            BlockItem.innerHTML = "назовите задачу";
            var i = event.target.id;
            console.log(i);
            ItemPlace[i].appendChild(BlockItem);
        }


        var ButtonCreateBlock = document.getElementsByClassName("block__button_unused");

        for (var i=0;i<ButtonCreateBlock.length; i++) {
            ButtonCreateBlock[i].onclick = createBlock;
        }

        function createBlock() {
            var Block = document.getElementById("block_create");
            var ButtonUnused = document.getElementById("block__button_unused");
            var newButtonUL = document.createElement("ul");
            newButtonUL.className = "blocks-folder__block__list";
            var newButton = document.createElement('div');
            newButton.className = "blocks-folder__block__button";
            newButton.id = IndexOfBlock;
            newButton.innerHTML = "Добавить карточку";


            Block.className = "blocks-folder__block blocks-folder__block_droppable";
            Block.removeChild(ButtonUnused);
            Block.appendChild(newButtonUL);
            Block.appendChild(newButton);

            var ButtonUse = document.getElementsByClassName("blocks-folder__block__button");

            for (var i=0;i<ButtonUse.length; i++) {
                ButtonUse[i].onclick = createBlockItem
            }
            CreateNewUnusedBlock();

            IndexOfBlock++;
        }

        function CreateNewUnusedBlock() {
            var BlocksFolder = document.getElementById("BlocksFolder");
            var newUnusedBlock = document.createElement("div");
            newUnusedBlock.className = "block_unused_droppable";
            newUnusedBlock.id = "block_create";
            var BlockButtonUnused = document.createElement("div");
            BlockButtonUnused.className = "block__button_unused";
            BlockButtonUnused.id = "block__button_unused";
            BlockButtonUnused.innerHTML = "Добавить блок";
            BlockButtonUnused.onclick = createBlock;

            BlocksFolder.appendChild(newUnusedBlock);
            newUnusedBlock.appendChild(BlockButtonUnused);

        }*/
   /* var IndexOfBlock = 2;
    var ItemPlace = document.getElementsByClassName("blocks-folder__block__list");
    var BlocksFolder = document.getElementById("BlocksFolder");
    BlocksFolder.addEventListener('click', createBlock);
    function createBlock(event) {

        if (event.target.id == "block__button_unused") {
            var Block = document.getElementById("block_create");
            var ButtonUnused = document.getElementById("block__button_unused");
            var newButtonUL = document.createElement("ul");
            newButtonUL.className = "blocks-folder__block__list";
            ...
            Block.className = "b locks-folder__block";
            Block.removeAttribute("id");
            Block.removeChild(ButtonUnused);
            Block.appendChild(newButtonUL);
            Block.appendChild(newButton);

            var BlFolder = document.getElementById("BlocksFolder");
            var newUnusedBlock = document.createElement("div");
            newUnusedBlock.className = "block_unused_droppable";
            newUnusedBlock.id = "block_unused";
            var BlockButtonUnused = document.createElement("div");
            BlockButtonUnused.className = "block__button_unused";
            BlockButtonUnused.id = "block__button_unused";
            BlockButtonUnused.innerHTML = "Добавить блок";

            BlFolder.appendChild(newUnusedBlock);
            newUnusedBlock.appendChild(BlockButtonUnused);
        } else if(event.target.className == "blocks-folder__block__button"){
            var BlockItem = document.createElement("li");
            BlockItem.className = "blocks-folder__block__list";
            BlockItem.innerHTML = "назовите задачу";
            var i = event.target.id;
            console.log(i);
            ItemPlace[i].appendChild(BlockItem);
...
/*});*/
/*  var Block = document.getElementById("block_unused");
            var ButtonUnused = document.getElementById("block__button_unused");
            var newButtonUL = document.createElement("ul");
            newButtonUL.className = "block__list";
            var newButton = document.createElement('div');
            newButton.className = "block__button";
            newButton.id = IndexOfBlock;
            newButton.innerHTML = "Добавить карточку";


            Block.className = "block_droppable";
            Block.removeChild(ButtonUnused);
            Block.appendChild(newButtonUL);
            Block.appendChild(newButton);

            var ButtonUse = document.getElementsByClassName("block__button");
